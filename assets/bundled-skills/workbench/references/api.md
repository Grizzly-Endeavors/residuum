...
| `DELETE /api/workspace/file?path=<p>&recursive=<bool>` | Deletes a file, or a directory when `recursive=true` (a directory without it answers `409`). The workspace root can't be deleted (`400`). `If-Match` applies to files. Returns `{ deleted: true }`. |
...
| `POST /api/workspace/move` | Body `{ from, to, overwrite? }` moves or renames a file or directory, creating `to`'s missing parents. `409` if `to` already exists and `overwrite` isn't `true`. `If-Match` applies to `from` when it's a file. Returns `{ moved: true, version }` (`version` is `null` for a moved directory). |
