...
5. After the write succeeds, a background watcher picks up the change and reloads automatically — there is no tool-level confirmation of this in `write_file`/`edit_file`'s own result. The reload's outcome ("configuration reloaded: ..." or "config reload failed: ...") is published only to the user's interface (a notice in the web UI, chat, etc.), not into your own conversation, so you cannot check it directly. Before writing, re-read the edited section to make sure the TOML is well-formed (balanced quotes/brackets/tables, correct value types) — a malformed file fails the reload and Residuum keeps running on the previous config until it's fixed.
6. Tell the user what changed — name the section, the key, and the new value, not just "updated the config" — and mention that Residuum applies it automatically within moments; ask them to let you know if they see a reload-failed notice.
