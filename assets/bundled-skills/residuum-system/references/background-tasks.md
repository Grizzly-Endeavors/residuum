...
Every session has a category — `scheduled` (pulses, actions), `external` (webhooks, and non-owner-DM conversations on Discord/Telegram/Teams — see Conversation Routing below), `spawned` (`subagent_spawn`, the `learner`), or `artifact` (started by a workbench artifact — see Artifact Sessions below) — and moves through `forking` → `queued` → `running` → `idle` → (`queued` → `running` → `idle` again, per later turn) → `completing` → `completed`. `queued` means the run is ready for a turn but waiting on a `max_concurrent` permit held by other live sessions. A run holds a concurrency permit only while `running`; it lingers `idle` for its category's timeout (`idle_timeout_scheduled_minutes` / `_spawned_minutes` / `_external_minutes` / `_artifact_minutes` in `[background]`, defaulting to 2 / 10 / 30 / 10 minutes; a webhook session uses the scheduled timeout) before completing. A `completing` run no longer accepts messages into itself — see Messaging below. `stop_agent` cancels a session's stop token: a running turn stops immediately — the model call aborts, the tool call in progress is interrupted, and every other tool call in that batch is skipped — with its transcript intact; an idle one completes immediately. Either way the run is reported as cancelled; a run that simply idles out keeps its last turn's outcome.
...
**Nesting**: sessions can spawn sessions via their own `subagent_spawn`, up to `subagent_depth_cap` (default 3; main is depth 0). See `subagent_spawn` Details below.
...
Delivery follows the same rules as Messaging below — interrupt if running, new turn if idle, deferred resume if completing — except a conversation message is always hop 0, and an address with no prior run starts a brand-new session instead of erroring as unknown. The session sees the same `[From: name via interface (location)]` attribution and buffered chatter each interface page describes. Unlike a busy `message_agent` send, which errors back to the sender, a conversation message that arrives while its session's interrupt channel is saturated is never dropped: it retries on a detached task until the channel drains, since there's nobody on the other end of a chat message to hand a refusal to.
...
The owner sees sessions in the web UI's sessions sidebar: live sessions with their category, source label, state, and running time, plus a paginated list of finished runs. Every row in a stoppable state (forking, queued, running, idle) carries its own stop button, in every category, so the owner can stop one straight from the list; an artifact session's row also links to the artifact that started it. Opening a session shows its transcript and live activity, with a message box and a stop button. A message the owner sends from there arrives labelled as coming from the owner via the web UI; answer it in your response, which the owner reads in that view. Results a session relays to `main`, and `message_agent` calls to `main`, show in the main chat as compact items linking to the sending session.

A finished run's real outcome (completed/cancelled/failed, with the plain-language error and the fuller technical details behind it for a failure) is persisted with its record, so it shows correctly — details included, behind the same expandable toggle a live error uses — even after a reload or once paged into the older-runs list, not just "finished". A `scheduled` pulse run that started while its previous run was still live carries an overlap flag, visible on the run and in the web UI's Scheduled view (see `heartbeats` reference).
