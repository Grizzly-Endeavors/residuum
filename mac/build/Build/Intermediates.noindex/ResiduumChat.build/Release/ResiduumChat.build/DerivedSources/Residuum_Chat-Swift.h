...
#ifndef RESIDUUM_CHAT_SWIFT_H
#define RESIDUUM_CHAT_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="Residuum_Chat",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
SWIFT_CLASS("_TtC13Residuum_Chat11AppDelegate")
...
@interface AppDelegate (SWIFT_EXTENSION(Residuum_Chat)) <NSWindowDelegate>
...
/// Call <code>connect()</code> to start. The connection retries automatically on
/// failure with exponential backoff (1 s → 2 s → 4 s → … → 30 s max).
/// Call <code>disconnect()</code> to stop permanently.
/// Received messages are delivered on the main queue via <code>onMessage</code>.
SWIFT_CLASS("_TtC13Residuum_Chat18ResiduumConnection")
@interface ResiduumConnection : NSObject
...
@class NSString;
@class NSData;
@interface ResiduumConnection (SWIFT_EXTENSION(Residuum_Chat)) <NSURLSessionWebSocketDelegate>
