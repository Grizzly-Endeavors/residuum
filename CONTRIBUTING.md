...
4. CI must pass before merge. The `CI OK` check sums it up. CI runs only the checks your changes need: Rust changes get formatting, clippy, tests, dependency audit, and cross-platform compile checks; `web/` changes get the web format, lint, type, and build checks; changes that only touch docs skip both. The path rules live in the `changes` job of `.github/workflows/pr.yml`, and any path they don't list runs every check.
