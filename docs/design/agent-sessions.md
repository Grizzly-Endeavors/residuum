...
A conversation session replies to its own conversation by default. The reply targeting and fallback behavior are unchanged (it can still use `send_message` to post elsewhere).
