...
**Results stay with the artifact.** An artifact session's turn output is never relayed to the main agent, and its completion is never routed to the inbox or notification channels. Its output reaches the artifact through the session frames (`session_response`, `session_broadcast_response`, `session_completed`, ...) and the transcript endpoint, and it appears in the sessions list like any other session. The session itself keeps its normal tools, so it can still message the main agent or file inbox items when its task calls for it.
...
| `POST /api/workbench/artifacts/{name}/stop` | Stops every live session started by that artifact. Answers `{ stopped: [addresses] }`. |
...
- **Artifact bar.** The bar above an open artifact shows its activity: live sessions it started (from the session registry, by artifact) and model calls in flight (tracked by the bridge, which relays them). A **Stop** control aborts the artifact's in-flight model calls, stops its live sessions via the artifact stop endpoint, and unloads the artifact's frame into a stopped state with a **Restart** button. Unloading ends any loop running in the page, which stopping its work alone would not.
...
- **Model-call tracking.** The bridge identifies model calls by route (`/api/model/complete`), gives each an abort signal, and tracks them per frame. It aborts all of them when the user presses Stop (§9) or when the bridge is torn down because the frame closes or navigates away.

### 11. Inbox, memory search, feature detection, state

- **Inbox.** `POST /api/inbox` with `{ title?, body }` adds an item to the user's inbox; `POST /api/agent-inbox` with the same body adds one to the agent's inbox (the same place the `/inbox` command writes). The title defaults to the body's first line, cut to 60 characters. The source is `artifact:<name>` (or `web` without the identity header). Both answer `{ id }`. Artifacts have no way to post into the main conversation.
...
- **Stop controls over caps.** The agent guardrail stance is default-open. Caps on concurrent calls or sessions would block legitimate batch work. A stop control that also unloads the page handles runaway loops without restricting normal use.
