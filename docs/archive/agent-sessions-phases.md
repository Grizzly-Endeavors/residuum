...
- **Shape when done:**
  ...
  - On completion, a run produces no episode only when it staged nothing and either its final turn ended in `HEARTBEAT_OK` or its transcript is under the configurable token floor. Otherwise the final observation runs and the merge writer commits the staged and final observations and the episode. Merged records carry the session address, run id, and category, and are optional when read so existing records still load.
