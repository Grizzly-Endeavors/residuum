...
- **Finished** is a collapsed section listing completed runs newest first, 25 at a time, with a "Show older" button that follows `next_cursor`. A reload keeps runs already paged in beyond the first page, matched by run id in the server's order. A run that finished while the page was open shows whether it was stopped or failed; the listing itself doesn't carry that, so older runs show "finished" (or "interrupted" when startup recovery closed them).
