...
| `session_completed` | `address`, `run_id`, `status` (`completed` \| `cancelled` \| `failed`), `error` (the failure when `failed`, else `null`), `error_details` (omitted when there's nothing richer than `error` to show), `episode_id` | The run was recorded in the store and is no longer live. There is no separate `completed` state change. |
...
| `session_error` | `address`, `run_id`, `message`, `details` (omitted when there's nothing richer than `message` to show) | A failed turn, a message refused at the hop limit (on both the sender's and the receiver's stream, when each is a live session), a result relay that couldn't be delivered, a deferred delivery that failed, or a panicked session task. |
...
- **Tasks sent to other agents** (`message_agent a2a:<name>`, from main or any session) are listed at the top of the External group while they're open, with an `a2a` badge, the remote agent's name, its latest status message, where the task stands, how long ago it was sent, and a stop button. They count toward the External group's and the header's live counts. They have no transcript here, so the row doesn't open a view. See [a2a.md](a2a.md#sending-and-receiving) for what stopping one does and the **Stop watching** fallback for an agent that can't be reached.
