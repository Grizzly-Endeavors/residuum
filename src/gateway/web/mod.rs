...
pub(super) fn config_api_router(state: ConfigApiState) -> axum::Router {
	axum::Router::new()
		...
		.route("/api/config/complete-setup", post(config::api_complete_setup))
		...
		.route("/api/providers/models", post(providers::api_provider_models))
		.route("/api/providers/raw", get(config::api_providers_raw_get))
		.route("/api/providers/raw", put(config::api_providers_raw_put))
		.route("/api/providers/validate", post(config::api_providers_validate))
...
mod tests {
	...
	async fn secrets_set_list_delete_roundtrip() {
		...
		// List secrets
		let list_result = secrets::api_secrets_list(State(state.clone())).await.unwrap();
