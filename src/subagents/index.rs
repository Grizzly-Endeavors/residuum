...
impl SubagentPresetIndex {
    ...
    ///
    /// # Errors
    /// Returns `FatalError::Subagents` if the directory cannot be read
    /// (except `NotFound`, which is silently skipped).
    pub async fn scan(dir: &Path) -> Result<Self, FatalError> {
    ...
    pub async fn load_preset(
        ...
        Err(FatalError::Subagents(format!(
