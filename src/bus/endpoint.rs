...
pub(crate) struct EndpointId(String);
...
pub(crate) struct EndpointCapabilities(u8);

impl EndpointCapabilities {
    /// Bidirectional conversation.
    pub(crate) const INTERACTIVE: Self = Self(0b0001);
    /// Supports streaming events (typing indicators, tool calls).
    pub(crate) const STREAMING: Self = Self(0b0010);
    /// Output-only push (notifications).
    pub(crate) const NOTIFY_ONLY: Self = Self(0b0100);
    /// Input-only, no response path.
    pub(crate) const INPUT_ONLY: Self = Self(0b1000);
