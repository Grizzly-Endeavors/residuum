...
pub async fn save_recent_context(path: &Path, ctx: &RecentContext) -> Result<(), IronclawError> {
    let json = serde_json::to_string_pretty(ctx)
        .map_err(|e| IronclawError::Memory(format!("failed to serialize recent context: {e}")))?;
...
pub async fn load_recent_context(path: &Path) -> Result<Option<RecentContext>, IronclawError> {
