...
impl VectorStore {
    ...
    pub fn open_or_create(db_path: &Path, dim: usize) -> Result<Self, FatalError> {
        ...
        conn.pragma_update(None, "journal_mode", "WAL")
            .map_err(|e| {
                FatalError::Memory(format!("failed to set WAL mode for vector store: {e}"))
            })?;
    ...
    pub fn insert_observations(
        ...
        let tx = conn
            .transaction()
            .map_err(|e| FatalError::Memory(format!("failed to start obs transaction: {e}")))?;
        ...
        let mut doc_ids = Vec::with_capacity(observations.len());
        {
            let mut stmt = tx
                ...
                .map_err(|e| FatalError::Memory(format!("failed to prepare obs insert: {e}")))?;
        ...
        tx.commit()
            .map_err(|e| FatalError::Memory(format!("failed to commit obs transaction: {e}")))?;
    ...
    pub fn insert_chunks(
        ...
        let tx = conn
            .transaction()
            .map_err(|e| FatalError::Memory(format!("failed to start chunk transaction: {e}")))?;
        ...
        let mut doc_ids = Vec::with_capacity(chunks.len());
        {
            let mut stmt = tx
                ...
                .map_err(|e| FatalError::Memory(format!("failed to prepare chunk insert: {e}")))?;
        ...
        tx.commit()
            .map_err(|e| FatalError::Memory(format!("failed to commit chunk transaction: {e}")))?;
    ...
    ) -> Result<Vec<VectorSearchResult>, FatalError> {
    ...
    pub fn delete_by_doc_ids(&self, ids: &[String]) -> Result<(), FatalError> {
        if ids.is_empty() {
            return Ok(());
        }
        ...
        let tx = conn
            .transaction()
            .map_err(|e| FatalError::Memory(format!("failed to begin delete transaction: {e}")))?;

        for id in ids {
            // Try both tables — a given ID only exists in one
            tx.execute("DELETE FROM obs_vectors WHERE obs_id = ?1", [id])
                .map_err(|e| {
                    FatalError::Memory(format!("failed to delete from obs_vectors: {e}"))
                })?;
            tx.execute("DELETE FROM chunk_vectors WHERE chunk_id = ?1", [id])
                .map_err(|e| {
                    FatalError::Memory(format!("failed to delete from chunk_vectors: {e}"))
                })?;
        }

        tx.commit()
            .map_err(|e| FatalError::Memory(format!("failed to commit delete transaction: {e}")))?;
    ...
    /// # Errors
    /// Returns an error if the query fails.
    pub fn has_observation(&self, obs_id: &str) -> Result<bool, FatalError> {
        let conn = self.lock_conn()?;
        let mut stmt = conn
            .prepare_cached("SELECT 1 FROM obs_vectors WHERE obs_id = ?1 LIMIT 1")
            .map_err(|e| {
                FatalError::Memory(format!("failed to prepare obs existence check: {e}"))
            })?;
        let exists = stmt
            .exists(rusqlite::params![obs_id])
            .map_err(|e| FatalError::Memory(format!("obs existence check failed: {e}")))?;
        Ok(exists)
    }
    ...
    /// # Errors
    /// Returns an error if the query fails.
    pub fn has_chunk(&self, chunk_id: &str) -> Result<bool, FatalError> {
        let conn = self.lock_conn()?;
        let mut stmt = conn
            .prepare_cached("SELECT 1 FROM chunk_vectors WHERE chunk_id = ?1 LIMIT 1")
            .map_err(|e| {
                FatalError::Memory(format!("failed to prepare chunk existence check: {e}"))
            })?;
        let exists = stmt
            .exists(rusqlite::params![chunk_id])
            .map_err(|e| FatalError::Memory(format!("chunk existence check failed: {e}")))?;
        Ok(exists)
    }
    ...
    pub fn clear(&self) -> Result<(), FatalError> {
        ...
        conn.execute_batch("DROP TABLE IF EXISTS obs_vectors; DROP TABLE IF EXISTS chunk_vectors;")
            .map_err(|e| FatalError::Memory(format!("failed to drop vector tables: {e}")))?;
        create_tables(&conn, self.dim)?;
        Ok(())
    }

    /// Lock the connection mutex.
    fn lock_conn(&self) -> Result<std::sync::MutexGuard<'_, Connection>, FatalError> {
        self.conn
            .lock()
            .map_err(|e| FatalError::Memory(format!("vector store lock poisoned: {e}")))
    }

    /// Validate that an embedding has the expected dimension.
    fn check_dim(&self, embedding: &[f32]) -> Result<(), FatalError> {
...
fn create_tables(conn: &Connection, dim: usize) -> Result<(), FatalError> {
    ...
    .map_err(|e| FatalError::Memory(format!("failed to create vector tables: {e}")))?;
    Ok(())
}
...
fn search_obs_table(
    ...
    let mut stmt = conn_ref
        .prepare_cached(&sql)
        .map_err(|e| FatalError::Memory(format!("failed to prepare obs search: {e}")))?;
    ...
    let rows = stmt
        ...
        .map_err(|e| FatalError::Memory(format!("obs vector search failed: {e}")))?;

    let mut results = Vec::new();
    for row in rows {
        results.push(
            row.map_err(|e| FatalError::Memory(format!("failed to read obs search row: {e}")))?,
        );
    }
    Ok(results)
}
...
fn search_chunk_table(
    ...
    let mut stmt = conn
        .prepare_cached(&sql)
        .map_err(|e| FatalError::Memory(format!("failed to prepare chunk search: {e}")))?;
    ...
    let rows = stmt
        ...
        .map_err(|e| FatalError::Memory(format!("chunk vector search failed: {e}")))?;

    let mut results = Vec::new();
    for row in rows {
        results.push(
            row.map_err(|e| FatalError::Memory(format!("failed to read chunk search row: {e}")))?,
        );
    }
    Ok(results)
}
