...
Owns the session registry, runtime, and store that execute work off the main agent's turn loop: pulse checks, scheduled actions, webhooks, on-demand sub-agent delegation, and work a workbench artifact starts. See `docs/systems-usage/background-tasks.md` for the user-facing behavior (the original design and its reasoning are archived in `docs/archive/agent-sessions-design.md`); this file covers how the module's own pieces fit together.
...
Every observable step of a run is published on the bus's `Sessions` topic as a `SessionEvent` (address, run id, and a `SessionEventKind`), through `events::publish_session_event()`, so the web UI can follow sessions without polling. `SessionRuntime::spawn()` publishes `Started` (carrying the full `SessionInfo`) before the run's first store write; `transition_state()` in `runtime.rs` pairs every registry state change (`Running`, `Idle`, `Completing`) with a `StateChanged`; `run_turn()` brackets each turn with `TurnStarted`/`TurnEnded` and publishes its `Response` or `Error` (turn ids are `<run_id>-t<n>`); `execute_subagent()` runs the turn with `EventTarget::Session`, so `execute_turn()` publishes tool calls, tool results, and intermediate text as session events instead of the main agent's endpoint events; `finish_run()` and `recover_from_panic()` publish `Completed` after the run leaves the registry. Hop-limit refusals, deferred-delivery failures (`messaging.rs`), and result-relay failures (`runtime.rs`) publish an `Error` on each affected live session, alongside the transcript note they already leave. A message delivered to the main agent (`AgentMessenger::deliver_to_main()`, which carries both turn-result relays and `message_agent` calls to `main`) publishes `MessageToMain` on the sending run's stream, so the web UI can show it in the main chat as it arrives. The gateway maps these to the `session_*` WebSocket frames (`crate::gateway::sessions`); a publish failure only means the broker is shutting down, so it is logged and never affects the run.

#### Agent Messaging

`message_agent` (`crate::tools::message_agent::MessageAgentTool`) is registered for both the main agent and every session, each instance carrying its own address, category, and a `HopCounter` clone so it can identify itself as the sender and compute the outgoing hop count (`hop_counter.outgoing()`, one more than the current turn's). An `artifact` session's instance refuses `to: main` with a tool error before sending anything (artifact sessions never reach the main conversation). Otherwise it calls `AgentMessenger::send()`, which:
...
`notify::router` subscribes to the bus `Background` topic and routes each `AgentResultEvent` by the disposition its session declared (via `HEARTBEAT_OK`/`HEARTBEAT_URGENT` sentinels in its summary — computed in `runtime.rs::build_result_event`, using the same rule the old bridge module used): `Silent` results are discarded; `spawned`-category results (`EventTrigger::Agent`) are discarded here too, since they were already relayed per-turn as above; `artifact`-category results (`EventTrigger::Artifact`) are discarded whatever their disposition, since they belong to the artifact that started them; everything else files to the inbox, plus every configured notification channel when `Urgent`.
...
- **`src/gateway/sessions.rs`** / **`src/gateway/web/sessions.rs`** / **`src/gateway/ws.rs`** / **`src/interfaces/websocket/subscriber.rs`** — the web sessions surface: map `SessionEvent`s to `session_*` WebSocket frames, list live sessions (`SessionRegistry::list_live()`) and completed runs (`SessionStore::list_completed_runs()`), serve transcripts (`SessionStore::read_run()`/`read_incremental_transcript()`), deliver the owner's sidebar messages through `AgentMessenger::send()` as sender `OWNER_ADDRESS` at hop 0 (and an artifact's HTTP messages as sender `artifact:<name>`, category `artifact`), stop sessions via `SessionRegistry::stop()`, and start `artifact` sessions by publishing a `SpawnRequestEvent` with `EventTrigger::Artifact`.
...
| `store.rs` | `SessionStore`, `RunRecord`, `RunTranscriptSink`, `RunCursor`: per-run metadata persistence, the incrementally-appended transcript file, the startup incomplete-run recovery sweep, and the paginated newest-first listing of completed runs (`list_completed_runs()`). |
...
| `events.rs` | `publish_session_event()`: the one place session lifecycle and turn events are published on the bus's `Sessions` topic. |
