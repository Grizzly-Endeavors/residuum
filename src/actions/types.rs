...
pub struct ScheduledAction {
    ...
    /// Agent routing: `None` = plain session, `Some(skill)` = session with
    /// that skill activated. `Some("main")` is rejected both by the
    /// `schedule_action` tool and when this store loads from disk.
