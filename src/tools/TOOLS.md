...
**Description sent to LLM:**
> Execute a shell command and return its output. Commands run via `sh -c` (Unix) or `cmd /C` (Windows) with a configurable timeout (default 120 seconds). To use a credential, name it in `keys` and reference its environment variable (e.g. $GITHUB_TOKEN); agent_keys_list shows what exists. To save a credential a command prints (e.g. a freshly minted token), set `store_output_as`: stdout is stored as an agent key and never shown to you.
